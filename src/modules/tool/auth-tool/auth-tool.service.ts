...
export class AuthToolService {
    ...
    randomToken(length: number = 10): string {
    ...
    validateUsernameOrEmail(username: string): boolean {
        return /^[A-Za-z0-9._-]{4,64}$/g.test(username) // username
            || /^[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/g.test(username); // email
    }
}
