...
export class QueryGetPipe implements PipeTransform<QueryPreOption, QueryPostOption> {
    ...
    transform(value: QueryPreOption, metadata: ArgumentMetadata): QueryPostOption {
        ...
        // Select fields
        const select = ((select: string): { [field: string]: 0 | 1 } => {
        ...
        const fields = ((sort: string): string[] => {
            if (!sort) return isDefault ? ["updatedAt"] : [];
            return sort.split(" ");

        })(value.sort);
        const orders = ((orderString: string, fields: string[]): Array<-1 | 1> => {
            const orders: Array<-1 | 1> = orderString ? orderString.split(" ").map(ord => ord === "-1" ? -1 : 1) : [];
            for (let i = orders.length; i < fields.length; ++i) {
                orders.push(this.defaultSortOrder(fields[i]));
            }
            return orders;
        })(value.order, fields);
