 ...
 */
export const CacheNoArgs = (
    { key, maxAge }: { key?: string, maxAge?: TTLFunction },
) => {
    return (
...
export const CacheArgs = (maxAge?: TTLFunction) => {
    return (
        ...
        const method = descriptor.value;
        descriptor.value = function(...args: any[]) {
            const key = `${target.name || target.constructor?.name}|${propertyKey}|${args.map(arg => JSON.stringify(arg)).join("|")}`;
            let result = cache.get(key);
            if (!result) {
                result = method.apply(this, args);
                maxAge ? cache.set(key, result, maxAge()) : cache.set(key, result);
